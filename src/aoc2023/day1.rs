...
pub fn part1() -> u32 {
    INPUT
        ...
        .map(|line| {
            ...
            String::from_iter([last, first].into_iter())
                .parse::<u32>()
                .unwrap()
        })
        .sum()
...
fn parse_digit(s: &str) -> u32 {
    if let Some(x) = s.chars().next().unwrap().to_digit(10) {
        x
    } else {
        DIGIT_WORDS
            .iter()
            .position(|digit_word| digit_word.contains(s))
            .unwrap() as u32
            + 1
    }
}
...
pub fn part2() -> u32 {
    INPUT
        ...
        .map(|line| {
            ...
            let mut x: Vec<Vec<(usize, &str)>> = DIGIT_WORDS
                .iter()
                .map(|digit_word| line.match_indices(digit_word).collect())
                .collect();
