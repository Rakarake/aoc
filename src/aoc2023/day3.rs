...
fn is_digit_part_digit(m: &Vec<Vec<Tile>>, pos: (usize, usize)) -> bool {
    ...
    // Left column
    if is_tile_that(m, (x - 1, y - 1), Tile::Symbol) {
        return true;
    }
    if is_tile_that(m, (x - 1, y + 0), Tile::Symbol) {
        return true;
    }
    if is_tile_that(m, (x - 1, y + 1), Tile::Symbol) {
        return true;
    ...
    // Top and bottom
    if is_tile_that(m, (x + 0, y - 1), Tile::Symbol) {
        return true;
    }
    if is_tile_that(m, (x + 0, y + 1), Tile::Symbol) {
        return true;
...
fn make_number(digits: &Vec<u32>) -> u32 {
    digits
        ...
        .rev()
        .enumerate()
        .map(|(exponent, d)| d * 10_u32.pow(exponent as u32))
        .sum()
}
...
pub fn part1() -> u32 {
    ...
    m.iter()
        .enumerate()
        .map(|(y, line)| {
            line.iter().enumerate().fold(0, |acc, (x, tile)| {
                match tile {
                    Tile::Empty => acc,
                    Tile::Symbol => acc,
                    Tile::Number(n) => {
